...
async function loadPublicList(): Promise<Movie[]> {
  try {
    ...
    const data = JSON.parse(raw) as Array<{ id: number; title: string; year?: number; popularity?: number; poster_path?: string; vote_count?: number; vote_average?: number; revenue?: number }>;
...
export async function GET() {
  ...
  // Prefer titles with many votes; fallback to popularity
